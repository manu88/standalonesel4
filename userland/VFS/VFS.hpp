...
typedef struct {
  ...
  bool isFile() const noexcept { return (type & 0xF000) == INODE_TYPE_FILE; }
...
public:
  ...
  struct Operations {
    ...
    virtual bool readBlock(const FileSystem &, uint8_t *, size_t, uint32_t) {
      return false;
    }
  };

  struct FileSystem {
    bool read(inode_t *ino, uint32_t inodeID) const {
      return ops->read(*this, ino, inodeID);
    }
    bool readBlock(uint8_t *buf, size_t bufSize, uint32_t blockID) const {
      return ops->readBlock(*this, buf, bufSize, blockID);
    }
    FileSystem(int = 0) {}
    ext2_priv_data priv;
