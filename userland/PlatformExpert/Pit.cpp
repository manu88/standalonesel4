// from
// https://github.com/seL4/util_libs/blob/master/libplatsupport/src/plat/pc99/pit.c
#include "Pit.hpp"
...
#define PIT_TICKS_TO_NS(ticks)                                                 \
  ((uint32_t)(ticks) * (NS_IN_S / TICKS_PER_SECOND))
...
#define PIT_IOPORT_CHANNEL(x)                                                  \
  (0x40 +                                                                      \
   x) /* valid channels are 0, 1, 2. we'll be using 0 exclusively, though */
...
/* helper functions */
static inline seL4_Error setPitMode(seL4_SlotPos portCap, uint8_t channel,
                                    uint8_t mode) {
  auto ret = seL4_X86_IOPort_Out8(
      portCap, PIT_IOPORT_PITCR,
      PITCR_SET_CHANNEL(
          PITCR_SET_OP_MODE(PITCR_SET_ACCESS_MODE(0, PITCR_ACCESS_LOHI), mode),
          channel));
  return ret;
}
...
bool PIT::configure(uint8_t mode, uint64_t ns) {
  ...
  /* program timeout */
  error = seL4_X86_IOPort_Out8(_portCap, PIT_IOPORT_CHANNEL(0),
                               (uint8_t)(ticks & 0xFF));
  if (error != seL4_NoError) {
