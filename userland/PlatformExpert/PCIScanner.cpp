...
uint32_t PCIDevice::getBaseAddr32(int index) const{
    ...
    if ((baddr & 0xFFFFFFFFUL) != baddr) {
        kprintf("WARNING: get_baseaddr32 called for 64-bit address. Address will be truncated.\n");
        ...
        if (compact) {
            ...
            if (dev.cfg.base_addr_space[i] == PCI_BASE_ADDRESS_SPACE_IO) {
                ...
                kprintf("    BAR%d : [ mem 0x%X sz 0x%x szmask 0x%x %s %s ]\n", i,
                       ...
                       dev.cfg.base_addr_size[i],
                       dev.cfg.base_addr_size_mask[i],
                       dev.cfg.base_addr_type[i] == PCI_BASE_ADDRESS_MEM_TYPE_64 ? "64bit" : "",
                       dev.cfg.base_addr_prefetchable[i] ? "prefetch" : "");
            }
        } else {
            /* Very verbose and space wasting debug output. */
            kprintf("    BASE_ADDR[%d] ----\n", i);
            if (dev.cfg.base_addr[i] == 0 || dev.cfg.base_addr_64H[i]) continue;
            kprintf("        base_addr_space[%d]: 0x%x [%s]\n", i, dev.cfg.base_addr_space[i],
                   dev.cfg.base_addr_space[i] ? "PCI_BASE_ADDRESS_SPACE_IO" :
                                                 "PCI_BASE_ADDRESS_SPACE_MEMORY");
            kprintf("        base_addr_type[%d]: 0x%x [ ", i, dev.cfg.base_addr_type[i]);
            if (dev.cfg.base_addr_type[i] == PCI_BASE_ADDRESS_MEM_TYPE_32) kprintf("32bit ");
            if (dev.cfg.base_addr_type[i] == PCI_BASE_ADDRESS_MEM_TYPE_64) kprintf("64bit ");
            if (dev.cfg.base_addr_type[i] == PCI_BASE_ADDRESS_MEM_TYPE_1M) kprintf("<1M ");
            kprintf("]\n");
            kprintf("        base_addr_prefetchable[%d]: %s\n", i, dev.cfg.base_addr_prefetchable[i]
                   ? "yes" : "no");
            kprintf("        base_addr[%d]: 0x%X\n", i, dev.getBaseAddr(i));
            kprintf("        base_addr_size_mask[%d]: 0x%x\n", i, dev.cfg.base_addr_size_mask[i]);
        }
    }
}
...
#define MASK(n) \
    __extension__ ({  __typeof__ (n) _n = (n); \
        (void)assert((unsigned long)_n <= (sizeof(unsigned long) * 8)); \
        (void)assert(_n > 0); \
        MASK_UNSAFE(_n - 1) | BIT(_n - 1); \
    })

uint32_t PCIScanner::readReg32(uint8_t bus, uint8_t dev, uint8_t fun, uint8_t reg){
  reg &= ~MASK(2);
  out32(_pciAddressSlot, PCI_CONFIG_ADDRESS, 0x80000000 | bus << 16 | dev << 11 | fun << 8 | reg);
  return in32(_pciDataSlot, PCI_CONFIG_DATA);
}

void PCIScanner::writeReg32(uint8_t bus, uint8_t dev, uint8_t fun, uint8_t reg, uint32_t val){
  reg &= ~MASK(2);
  out32(_pciAddressSlot, PCI_CONFIG_ADDRESS, 0x80000000 | bus << 16 | dev << 11 | fun << 8 | reg);
...
void PCIScanner::out32(seL4_CPtr slot, uint32_t port_no, uint32_t val){
  auto err =
    seL4_X86_IOPort_Out32(slot, port_no, val);
  if (err != seL4_NoError) {
...
uint32_t PCIScanner::in32(seL4_CPtr slot, uint32_t port_no){
  seL4_X86_IOPort_In32_t val =
      seL4_X86_IOPort_In32(slot, port_no);
  if (val.error != seL4_NoError) {
...
void PCIScanner::readIOConfig(PCIDevice::IOConfig &cfg, uint8_t bus, uint8_t dev, uint8_t fun)
{
    for (int i = 0; i < 6; i++) {
        ...
        uint32_t cfg_base_addr = readReg32(bus, dev, fun, PCI_BASE_ADDRESS_0 + (i * 4));
