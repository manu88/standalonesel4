...
uint16_t PCIScanner::pciConfigReadWord(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset){
    ...
    auto err = seL4_X86_IOPort_Out32(_pciAddressSlot, PCI_CONFIG_ADDRESS, address);
    if(err != seL4_NoError){
        kprintf("error seL4_X86_IOPort_Out8 %i\n", err);
    }
    assert(err == seL4_NoError);

    seL4_X86_IOPort_In32_t val = seL4_X86_IOPort_In32(_pciDataSlot, PCI_CONFIG_DATA);
    if(err != seL4_NoError){
...
void PCIScanner::scan(){    
    for(int bus=0; bus<256;bus++){
        for(int slot=0; slot<256;slot++){
            ...
            _devices.push_back({.bus = bus, .slot = slot, .vendorID = vendorID, .deviceID = deviceID, .subclassCode = subClassAndClass.fields.subclassCode, .classCode = subClassAndClass.fields.classCode});
